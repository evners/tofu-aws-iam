...
| <a name="input_groups"></a> [groups](#input\_groups) | List of IAM groups to create, including managed and inline policies | <pre>list(object({<br>    name             = string<br>    path             = optional(string, "/")<br>    managed_policies = optional(list(string), [])<br>  }))</pre> | `[]` | no |
